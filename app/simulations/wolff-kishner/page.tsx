...
export default function WolffKishnerPage() {
  ...
  useEffect(() => {
    ...
    const drawKetone = (ctx: CanvasRenderingContext2D, cx: number, cy: number, rot: number) => {
      ...
      drawAtom(ctx, cx + c1.x, cy + c1.y, c1.z, '#8b5cf6', 'R₁', 16);
      drawAtom(ctx, cx + c2.x, cy + c2.y, c2.z, '#8b5cf6', 'C', 16);
      drawAtom(ctx, cx + c3.x, cy + c3.y, c3.z, '#8b5cf6', 'C', 16);
      drawAtom(ctx, cx + c4.x, cy + c4.y, c4.z, '#8b5cf6', 'R₂', 16);
    ...
    const drawHydrazone = (ctx: CanvasRenderingContext2D, cx: number, cy: number, rot: number) => {
      const c1 = rotate3D(-100, 0, 0, 0, rot);
      const c2 = rotate3D(-40, 0, 0, 0, rot);
      const c3 = rotate3D(20, 0, 0, 0, rot);
      const c4 = rotate3D(80, 0, 0, 0, rot);
      const n1 = rotate3D(20, -45, 0, 0, rot);
      const n2 = rotate3D(20, -80, 0, 0, rot);
      const h1 = rotate3D(50, -80, 0, 0, rot);
      const h2 = rotate3D(-10, -80, 0, 0, rot);

      drawBond(ctx, cx + c1.x, cy + c1.y, c1.z, cx + c2.x, cy + c2.y, c2.z);
      drawBond(ctx, cx + c2.x, cy + c2.y, c2.z, cx + c3.x, cy + c3.y, c3.z);
      drawBond(ctx, cx + c3.x, cy + c3.y, c3.z, cx + c4.x, cy + c4.y, c4.z);
      drawBond(ctx, cx + c3.x, cy + c3.y, c3.z, cx + n1.x, cy + n1.y, n1.z, 'double', '#3b82f6');
      drawBond(ctx, cx + n1.x, cy + n1.y, n1.z, cx + n2.x, cy + n2.y, n2.z, 'single', '#3b82f6');
      drawBond(ctx, cx + n2.x, cy + n2.y, n2.z, cx + h1.x, cy + h1.y, h1.z);
      drawBond(ctx, cx + n2.x, cy + n2.y, n2.z, cx + h2.x, cy + h2.y, h2.z);

      drawAtom(ctx, cx + c1.x, cy + c1.y, c1.z, '#8b5cf6', 'R₁', 16);
      drawAtom(ctx, cx + c2.x, cy + c2.y, c2.z, '#8b5cf6', 'C', 16);
      drawAtom(ctx, cx + c3.x, cy + c3.y, c3.z, '#8b5cf6', 'C', 16);
      drawAtom(ctx, cx + c4.x, cy + c4.y, c4.z, '#8b5cf6', 'R₂', 16);
      drawAtom(ctx, cx + n1.x, cy + n1.y, n1.z, '#3b82f6', 'N', 16);
      drawAtom(ctx, cx + n2.x, cy + n2.y, n2.z, '#3b82f6', 'N', 16);
      drawAtom(ctx, cx + h1.x, cy + h1.y, h1.z, '#e2e8f0', 'H', 12);
      drawAtom(ctx, cx + h2.x, cy + h2.y, h2.z, '#e2e8f0', 'H', 12);

      ctx.fillStyle = '#3b82f6';
      ctx.font = 'bold 12px Arial';
      const label = rotate3D(20, -105, 0, 0, rot);
      ctx.fillText('Hydrazone', cx + label.x, cy + label.y);
    };

    const drawDeprotonated = (ctx: CanvasRenderingContext2D, cx: number, cy: number, rot: number) => {
      const c1 = rotate3D(-100, 0, 0, 0, rot);
      const c2 = rotate3D(-40, 0, 0, 0, rot);
      const c3 = rotate3D(20, 0, 0, 0, rot);
      const c4 = rotate3D(80, 0, 0, 0, rot);
      const n1 = rotate3D(20, -45, 0, 0, rot);
      const n2 = rotate3D(20, -80, 0, 0, rot);
      const h = rotate3D(50, -80, 0, 0, rot);

      drawBond(ctx, cx + c1.x, cy + c1.y, c1.z, cx + c2.x, cy + c2.y, c2.z);
      drawBond(ctx, cx + c2.x, cy + c2.y, c2.z, cx + c3.x, cy + c3.y, c3.z);
      drawBond(ctx, cx + c3.x, cy + c3.y, c3.z, cx + c4.x, cy + c4.y, c4.z);
      drawBond(ctx, cx + c3.x, cy + c3.y, c3.z, cx + n1.x, cy + n1.y, n1.z, 'double', '#10b981');
      drawBond(ctx, cx + n1.x, cy + n1.y, n1.z, cx + n2.x, cy + n2.y, n2.z, 'single', '#10b981');
      drawBond(ctx, cx + n2.x, cy + n2.y, n2.z, cx + h.x, cy + h.y, h.z);

      drawAtom(ctx, cx + c1.x, cy + c1.y, c1.z, '#8b5cf6', 'R₁', 16);
      drawAtom(ctx, cx + c2.x, cy + c2.y, c2.z, '#8b5cf6', 'C', 16);
      drawAtom(ctx, cx + c3.x, cy + c3.y, c3.z, '#8b5cf6', 'C', 16);
      drawAtom(ctx, cx + c4.x, cy + c4.y, c4.z, '#8b5cf6', 'R₂', 16);
      drawAtom(ctx, cx + n1.x, cy + n1.y, n1.z, '#10b981', 'N', 16);
      drawAtom(ctx, cx + n2.x, cy + n2.y, n2.z, '#10b981', 'N⁻', 18);
      drawAtom(ctx, cx + h.x, cy + h.y, h.z, '#e2e8f0', 'H', 12);

      ctx.fillStyle = '#10b981';
      ctx.font = 'bold 14px Arial';
      ctx.fillText('Base removes H⁺', cx, cy + 100);
    };

    const drawDiazene = (ctx: CanvasRenderingContext2D, cx: number, cy: number, rot: number) => {
      const c1 = rotate3D(-100, 0, 0, 0, rot);
      const c2 = rotate3D(-40, 0, 0, 0, rot);
      const c3 = rotate3D(20, 0, 0, 0, rot);
      const c4 = rotate3D(80, 0, 0, 0, rot);
      const h1 = rotate3D(20, -40, 0, 0, rot);
      const h2 = rotate3D(20, 40, 0, 0, rot);
      ...
      const n2 = rotate3D(100, -50, 0, 0, rot);
      drawAtom(ctx, cx + n2.x, cy + n2.y, n2.z, '#f59e0b', 'N₂', 18);

      ctx.fillStyle = '#f59e0b';
      ctx.font = 'bold 14px Arial';
      ctx.fillText('N₂ gas leaves!', cx + 110, cy - 50);

      const arrow = rotate3D(80, -30, 0, 0, rot);
      const arrowEnd = rotate3D(120, -50, 0, 0, rot);
      ctx.strokeStyle = '#f59e0b';
      ctx.lineWidth = 2;
      ctx.setLineDash([5, 5]);
      ctx.beginPath();
      ctx.moveTo(cx + arrow.x, cy + arrow.y);
    ...
    const drawAlkane = (ctx: CanvasRenderingContext2D, cx: number, cy: number, rot: number) => {
      ...
      drawBond(ctx, cx + c2.x, cy + c2.y, c2.z, cx + c3.x, cy + c3.y, c3.z, 'single', '#22c55e');
      drawBond(ctx, cx + c3.x, cy + c3.y, c3.z, cx + c4.x, cy + c4.y, c4.z, 'single', '#22c55e');
      drawBond(ctx, cx + c3.x, cy + c3.y, c3.z, cx + h1.x, cy + h1.y, h1.z);
      drawBond(ctx, cx + c3.x, cy + c3.y, c3.z, cx + h2.x, cy + h2.y, h2.z);

      drawAtom(ctx, cx + c1.x, cy + c1.y, c1.z, '#22c55e', 'R₁', 16);
      drawAtom(ctx, cx + c2.x, cy + c2.y, c2.z, '#22c55e', 'CH₂', 16);
      drawAtom(ctx, cx + c3.x, cy + c3.y, c3.z, '#22c55e', 'CH₂', 16);
      drawAtom(ctx, cx + c4.x, cy + c4.y, c4.z, '#22c55e', 'R₂', 16);
      drawAtom(ctx, cx + h1.x, cy + h1.y, h1.z, '#e2e8f0', 'H', 12);
      drawAtom(ctx, cx + h2.x, cy + h2.y, h2.z, '#e2e8f0', 'H', 12);
    };
    ...
    const draw = () => {
      ...
      if (step === 0) {
        ...
        const reagent1 = rotate3D(-100, 90, 0, 0, rotation);
        const reagent2 = rotate3D(100, 90, 0, 0, rotation);
        ctx.fillText('N₂H₄', centerX + reagent1.x, centerY + reagent1.y);
        ctx.fillText('KOH/Heat', centerX + reagent2.x, centerY + reagent2.y);
        ctx.font = '14px Arial';
        ctx.fillStyle = '#94a3b8';
        ctx.fillText('(Hydrazine)', centerX + reagent1.x, centerY + reagent1.y + 20);
        ctx.fillText('(Strong base)', centerX + reagent2.x, centerY + reagent2.y + 20);
      } else if (step === 1) {
        drawHydrazone(ctx, centerX, centerY - 30, rotation);
        ctx.fillStyle = '#3b82f6';
        ctx.font = 'bold 16px Arial';
        ctx.fillText('C=N-NH₂ formed', centerX, centerY + 100);
        const water = rotate3D(0, 120, 0, 0, rotation);
        drawAtom(ctx, centerX + water.x, centerY + water.y, water.z, '#38bdf8', 'H₂O', 18);
        ctx.fillStyle = '#94a3b8';
        ctx.font = '12px Arial';
        ctx.fillText('(water eliminated)', centerX, centerY + 145);
      } else if (step === 2) {
        drawDeprotonated(ctx, centerX, centerY - 20, rotation);
        const base = rotate3D(-120, 50, 0, 0, rotation);
        drawAtom(ctx, centerX + base.x, centerY + base.y, base.z, '#10b981', 'OH⁻', 18);
        
        // Arrow
        const arrowStart = rotate3D(-80, 50, 0, 0, rotation);
        const arrowEnd = rotate3D(-20, -60, 0, 0, rotation);
        ...
        drawDiazene(ctx, centerX, centerY - 20, rotation);
        ctx.fillStyle = '#f59e0b';
        ctx.font = 'bold 16px Arial';
        ctx.fillText('Nitrogen gas evolution', centerX, centerY + 110);
        ctx.font = '14px Arial';
        ctx.fillStyle = '#94a3b8';
        ctx.fillText('(Exergonic - drives reaction forward)', centerX, centerY + 130);
      } else if (step === 4) {
      ...
      ctx.textAlign = 'center';
      ctx.fillText('Chemical Equations - Wolff-Kishner Reduction', width / 2, boxY + 22);

      const equations = [
        'General Reaction:',
        "R-CO-R'  +  N₂H₄  →[KOH, Δ]  R-CH₂-R'  +  N₂ ↑",
        '',
        'Example (Cyclohexanone → Cyclohexane):',
        'C₆H₁₀O  +  N₂H₄  →[KOH, 200°C]  C₆H₁₂  +  N₂  +  H₂O',
        '',
        'Mechanism Steps:',
        '1. Hydrazone formation: R₂C=O + N₂H₄ → R₂C=N-NH₂ + H₂O',
        '2. Base deprotonation: R₂C=N-NH₂ + OH⁻ → R₂C=N-NH⁻',
        '3. N₂ loss & protonation: R₂C=N-NH⁻ → R₂CH₂ + N₂'
      ];
    ...
    <div className="relative w-full h-screen overflow-hidden bg-[#020617]">
      ...
      <div className="absolute top-0 left-0 p-6 z-10 w-full flex justify-between items-start pointer-events-none">
        <div>
            ...
            <h1 className="text-4xl font-bold text-white mb-1">Wolff-Kishner Reduction</h1>
            <p className="text-teal-400/80 text-sm">Reduction of ketones/aldehydes to alkanes using Hydrazine</p>
        </div>
      ...
      {showInfo && (
        <div className="absolute top-24 right-8 w-96 bg-slate-900/95 backdrop-blur-md border border-slate-700 p-6 rounded-2xl shadow-2xl z-20">
          ...
          <p className="text-slate-300 text-sm mb-4 leading-relaxed">
            The Wolff-Kishner reduction converts ketones/aldehydes to alkanes using hydrazine (N₂H₄) and strong base (KOH/NaOH) under heat.
          </p>
          
          <div className="space-y-3">
             <div className="text-sm border-l-2 border-teal-500 pl-3">
               <strong className="text-teal-400 block mb-1">Key Features:</strong>
               <span className="text-slate-400 block mb-1">Basic conditions (suitable for acid-sensitive compounds).</span>
               <span className="text-slate-400 block">Evolution of N₂ gas drives the reaction forward (Exergonic).</span>
             </div>
          </div>
        </div>
      )}
